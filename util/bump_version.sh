...
perform_release() {
	...
	sed -i '/\<release/s/\(version\)="[^"]*"/\1="'"$release_version"'"/' misc/it.matematicasuperpiatta.minetest.appdata.xml
	sed -i 's/\(<release date\)="[^"]*"/\1="'"$RELEASE_DATE"'"/' misc/it.matematicasuperpiatta.minetest.appdata.xml

	git add -f misc/it.matematicasuperpiatta.minetest.appdata.xml
